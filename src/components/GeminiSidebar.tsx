...
import { Bot, Send, Loader2, X, Sparkles, MessageSquare, Info } from "lucide-react";
...
export default function GeminiSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
...
Răspunde concis, prietenos și util în limba română.`;
  ...
  return (
    <>
      {/* Overlay */}
      {open && <div className="gemini-overlay" onClick={onClose} />}
      
      {/* Sidebar */}
      <aside className={`gemini-sidebar ${open ? 'open' : ''}`}>
        <div className="gemini-header">
          <div className="gemini-header-title">
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700 }}>Asistent Gemini</div>
              <div style={{ fontSize: 11, color: "var(--tx-3)", display: "flex", alignItems: "center", gap: 4 }}>
                <Sparkles size={10} color="var(--ac)" /> Inteligență artificială activă
              </div>
            </div>
          </div>
        ...
        <div className="gemini-footer">
          ...
          <div className="gemini-footer-note">
            <Info size={10} /> Modelele AI pot face erori. Verifică informațiile critice.
          </div>
        </div>
      </aside>
      ...
      <style>{`
        ...
        .gemini-header-title { display: flex; align-items: center; gap: 12px; }
        .gemini-icon-box {
          width: 36px; height: 36px; border-radius: 10px; background: var(--ac-dim);
        ...
        .gemini-close-btn {
          ...
          padding: 8px; border-radius: var(--r-md); transition: all 0.2s;
        }
        ...
        .gemini-empty {
          margin-top: 40px; text-align: center; padding: 0 20px;
        }
        .gemini-empty-icon {
        ...
        .gemini-empty h3 { font-size: 18px; margin-bottom: 8px; color: var(--tx-1); }
        .gemini-empty p { font-size: 13px; color: var(--tx-3); line-height: 1.5; }
        
        .gemini-suggestions { margin-top: 24px; display: flex; flex-direction: column; gap: 8px; }
        .gemini-suggestions button {
          background: var(--bg-2); border: 1px solid var(--border);
          padding: 10px 14px; border-radius: var(--r-md); text-align: left;
        ...
        .gemini-msg { display: flex; flex-direction: column; max-width: 85%; }
        .gemini-msg.user { align-self: flex-end; }
        .gemini-msg.ai { align-self: flex-start; }
        
        .gemini-msg-bubble {
          padding: 12px 16px; border-radius: 16px; font-size: 13px; line-height: 1.6;
        ...
        .gemini-input {
          width: 100%; min-height: 44px; background: var(--bg-base);
          border: 1px solid var(--border); border-radius: 18px;
          padding: 10px 48px 10px 16px; font-size: 13px; color: var(--tx-1);
        ...
        .gemini-footer-note {
          margin-top: 10px; text-align: center; font-size: 10px; color: var(--tx-4);
